...
function PriceListDetailPage() {
  ...
  const linesQuery = useQuery({
    ...
    queryFn: () =>
      api.get<{ items: PriceListLineApi[] }>(`/api/pricing/lists/${id}/lines`),
  ...
  const auditQuery = useQuery({
    ...
    // 4xx (ex.: 403 para roles sem permissão de auditoria) não devem repetir.
    retry: (count: number, err: ApiError) => err.status >= 500 && count < 2,
  });
      ...
      {activeTab === 'auditoria' && (
        <Card padding="none">
          ...
          {auditQuery.isLoading ? (
            ...
            (auditQuery.error as ApiError).status === 403 ? (
              <div className="px-4 py-8 text-center text-sm text-neutral-500">
                Sem permissão para ver o histórico de alterações.
              </div>
            ) : (
